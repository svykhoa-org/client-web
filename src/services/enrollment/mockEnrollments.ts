...
export const mockEnrolledCourseIds = mockCourses.slice(0, 10).map(course => course.id)
...
export const getEnrolledCourses = (): Course[] => {
  return mockCourses.filter(course => mockEnrolledCourseIds.includes(course.id))
}
